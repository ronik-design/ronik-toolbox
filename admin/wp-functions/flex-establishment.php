...
?>
    <div data-flex-establishment="outter" <?= ronikdesigns_AdvancedSpacingSettings('flex', $f_pageID); ?> id="<?php echo esc_attr($id . ' ' . $f_id_outer); ?>" class="<?php echo esc_attr($class_name . ' ' . $f_cl_outer . ' ' . $f_color_override); ?>" style="<?= ' position:relative; ' . $f_bg . $f_img . ' ' . $f_inlineStyle . ' ' . ronikdesigns_AdvancedSettings('flex', $f_pageID); ?>">
        <?php if (isset($f_gradient_overlay['enable_gradient']) && $f_gradient_overlay['enable_gradient']) {
            echo $gradient_overlay;
        } ?>
        <div data-flex-establishment="inner" style="z-index:1; width:100%; <?= ronikdesigns_AdvancedSettingsInner('flex', $f_pageID); ?>" class="<?php echo esc_attr($flexname_inner . ' ' . $f_cl_inner); ?>" id="<?php echo esc_attr($f_id_inner); ?>">
            <?php
            if ($f_mod) {
                $f_content('mod');
            } else {
